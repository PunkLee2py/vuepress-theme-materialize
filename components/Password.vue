        ...
        <template #main>
            <main class="container">
                <div class="main-wrapper row center">
                    <div class="col s12">
                        <div
                            class="card-panel duration-10"
                            :class="passwordClass"
                        >
                            <div
                                class="inner-wrap"
                                :style="disabled ? 'z-index:1000' : ''"
                            >
                                <Preloader
                                    progress
                                    :class="{ progress: disabled }"
                                />
                            </div>
                            <div class="row">
                                <h4>
...
export default {
    ...
    mounted() {
        if (
            !this.password_hash ||
            this.password_hash === this._getStoragePWD()
        ) {
        ...
        this.passwordErrorClass =
            this.$frontmatter.passwordErrorClass || "shakeX";
    },
    watch: {
        disabled(val) {
            if (val) {
